...
def plot_path_range(x_list, y_list, yaw_list, ax, width, color, alpha=0.3):
    ...
    ax.fill(np.concatenate([path_left_x, path_right_x[::-1]]),\
             np.concatenate([path_left_y, path_right_y[::-1]]), alpha=alpha, color=color)